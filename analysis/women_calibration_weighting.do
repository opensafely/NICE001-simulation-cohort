...
* Parental hip fracture is intentionally excluded from calibration.
...
foreach target_matrix in N_age C_lowbmi C_prevfx C_smoke C_alcohol ///
    C_ra C_steroid C_secondary M_bmi SD_bmi {
    forvalues g = 1/9 {
...
local binary_target_matrices P_prevfx P_steroid P_ra P_smoke P_secondary P_alcohol P_lowbmi
...
local cali_binary_target_matrices `full_binary_target_matrices' `bmi_binary_target_matrices'
...
save "output/women_calibration_feasibility.dta", replace
export delimited using "output/women_calibration_feasibility.csv", replace
restore

if `n_feasibility_failures' > 0 {
    display as error "`n_feasibility_failures' feasibility checks failed."
    display as error "See output/women_calibration_feasibility.csv."
}
...
label variable calibration_weight "Women calibration weight: exact age + broad risks + BMI mean/low BMI"
